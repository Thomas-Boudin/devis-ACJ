(() => {
  ...
  function attach() {
    ...
    observer = new MutationObserver((mutations) => {
      const planningChanged = mutations.some((mutation) => [...mutation.addedNodes, ...mutation.removedNodes].some((node) => node instanceof Element && (node.matches?.('.acjPlanningScroll,.acjPlanningSummary') || node.querySelector?.('.acjPlanningScroll,.acjPlanningSummary'))));
      if (planningChanged) scheduleRender();
    });
