// Assistant ACJ v16 — texte + photos + historique Ogust anonymisé.
(function(){
  ...
  function addStyles(){
    if(document.getElementById('ai-v16-style')) return;
    const style=document.createElement('style');
    style.id='ai-v16-style';
    style.textContent=`
      ...
      .aiVisual{margin-top:8px;padding:9px 10px;border:1px solid #1d4ed8;background:#0b1d3a;border-radius:11px;color:#bfdbfe;font-size:10px;line-height:1.45}.aiVisual strong{color:#dbeafe}.aiHistory{margin-top:8px;padding:9px 10px;border:1px solid #6d28d9;background:#1c1235;border-radius:11px;color:#ddd6fe;font-size:10px;line-height:1.45}.aiHistory strong{color:#ede9fe}.aiHistoryMeta{margin-top:3px;color:#c4b5fd}.aiHistoryStatus{font-size:10px;color:#9fb8c8;margin-top:9px;line-height:1.35}
  ...
  function historyHtml(p){
    ...
    return `<div class="aiHistory"><strong>Historique Ogust</strong><br>${esc(bits.join(' · '))}${p.history_note?`<div class="aiHistoryMeta">${esc(p.history_note)}</div>`:''}</div>`;
  }

  function historyStatusHtml(){
    const h=lastMeta?.ogust_history;
    if(!h) return '';
    if(h.available&&Number(h.record_count)>0) return `<div class="aiHistoryStatus">Historique Ogust consulté : ${Math.round(Number(h.record_count))} interventions récentes disponibles pour rechercher des cas comparables.</div>`;
    ...
    return `<div class="aiHistoryStatus">Historique Ogust non disponible pour cette analyse : estimation basée sur les informations du chantier et les repères ACJ.</div>`;
  }
  ...
  function injectCard(){
    ...
    card.innerHTML=`
      ...
      <div class="aiHelp">Décris le chantier et ajoute jusqu’à 4 photos. Les photos servent à reconnaître l’état, l’accès et la difficulté ; les dimensions précises restent à confirmer. L’historique Ogust est consulté quand il contient des interventions comparables.</div>
      ...
      <div class="aiActions"><button id="aiAnalyseBtn" class="btn primary" type="button" onclick="analyseChantierAI()">Analyser le chantier</button><div id="aiLoader" class="aiLoader">Analyse du chantier et des photos…</div></div>
  ...
  function showEstimateNotice(p,usedHours){
    ...
    notice.innerHTML=`<strong>Temps conseillé prérempli : ${esc(durationLabel(usedHours))}</strong><br>Fourchette estimée : ${esc(durationLabel(min))} – ${esc(durationLabel(max))}<br>Fiabilité de l’estimation : ${reliability} %.${history}<br>Tu peux modifier les heures avant d’ajouter la prestation.`;
